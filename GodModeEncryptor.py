import os
import sys
import shutil
import base64
import customtkinter as ctk
from tkinter import filedialog, messagebox
from cryptography.fernet import Fernet
from cryptography.hazmat.primitives import hashes
from cryptography.hazmat.primitives.kdf.pbkdf2 import PBKDF2HMAC

# --- تعريف أنواع التشفير ---
MODE_PASSWORD_ONLY = b'\x01'
MODE_PASSWORD_AND_KEY = b'\x02'

# --- إعدادات المظهر ---
ctk.set_appearance_mode("Dark")
ctk.set_default_color_theme("green")

class AegisCrypt(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("AegisCrypt")
        self.geometry("700x600")
        self.attributes('-alpha', 0.0) # للأنيميشن

        # --- حل مشكلة الأيقونة ---
        try:
            icon_path = self.resource_path("icon.ico")
            self.iconbitmap(icon_path)
        except Exception as e:
            print(f"Warning: Could not load icon.ico. {e}")

        # --- تصميم الواجهة ---
        self.grid_columnconfigure(0, weight=1)
        
        self.header_label = ctk.CTkLabel(self, text="AegisCrypt", font=ctk.CTkFont(size=32, weight="bold", family="Segoe UI Black"))
        self.header_label.grid(row=0, column=0, padx=20, pady=(20, 10))
        self.after(200, self.fade_in) # بدء أنيميشن الظهور

        self.tabview = ctk.CTkTabview(self, width=680)
        self.tabview.grid(row=1, column=0, padx=10, pady=5, sticky="nsew")
        self.encrypt_tab = self.tabview.add("🔒   تشفير   ")
        self.decrypt_tab = self.tabview.add("🔑   فك تشفير   ")

        self.setup_encrypt_tab()
        self.setup_decrypt_tab()

        self.status_label = ctk.CTkLabel(self, text="الحالة: جاهز لحماية ملفاتك.", text_color="gray")
        self.status_label.grid(row=2, column=0, padx=20, pady=10, sticky="ew")
        self.progress_bar = ctk.CTkProgressBar(self, mode='indeterminate')

        # متغيرات التشغيل
        self.source_path = ""
        self.locked_file = ""
        self.key_file = ""
        self.decryption_mode = None

    def resource_path(self, relative_path):
        """ Get absolute path to resource, works for dev and for PyInstaller """
        try: base_path = sys._MEIPASS
        except Exception: base_path = os.path.abspath(".")
        return os.path.join(base_path, relative_path)
        
    def fade_in(self):
        """ أنيميشن الظهور السلس للنافذة """
        alpha = self.attributes('-alpha')
        if alpha < 1:
            alpha += 0.08
            self.attributes('-alpha', alpha)
            self.after(15, self.fade_in)

    def setup_encrypt_tab(self):
        self.encrypt_tab.grid_columnconfigure(0, weight=1)
        
        title_enc = ctk.CTkLabel(self.encrypt_tab, text="تشفير ملف أو مجلد", font=ctk.CTkFont(size=18, weight="bold"))
        title_enc.grid(row=0, column=0, columnspan=2, padx=20, pady=15)

        enc_frame = ctk.CTkFrame(self.encrypt_tab)
        enc_frame.grid(row=1, column=0, columnspan=2, padx=20, pady=10, sticky="ew")
        enc_frame.grid_columnconfigure(1, weight=1)
        
        select_button_enc = ctk.CTkButton(enc_frame, text="اختر...", width=100, command=self.select_path_to_encrypt)
        select_button_enc.grid(row=0, column=0, padx=15, pady=15)
        self.path_label_enc = ctk.CTkLabel(enc_frame, text="لم يتم اختيار أي شيء", text_color="gray", anchor="w")
        self.path_label_enc.grid(row=0, column=1, padx=15, pady=15, sticky="ew")

        self.password_entry_enc = ctk.CTkEntry(self.encrypt_tab, placeholder_text="أدخل كلمة مرور قوية جدًا", show="*", height=35)
        self.password_entry_enc.grid(row=2, column=0, columnspan=2, padx=20, pady=20, sticky="ew")
        
        self.use_keyfile_check = ctk.CTkCheckBox(self.encrypt_tab, text="أمان إضافي (مُوصى به): كلمة مرور + ملف مفتاح", font=ctk.CTkFont(weight="bold"))
        self.use_keyfile_check.grid(row=3, column=0, columnspan=2, padx=20, pady=10)
        self.use_keyfile_check.select()

        self.encrypt_button = ctk.CTkButton(self.encrypt_tab, text="🔒 ابدأ التشفير الآن", height=45, font=ctk.CTkFont(size=18, weight="bold"), command=self.encrypt_action)
        self.encrypt_button.grid(row=4, column=0, columnspan=2, padx=20, pady=(20,10), sticky="ew")

    def setup_decrypt_tab(self):
        self.decrypt_tab.grid_columnconfigure(0, weight=1)
        
        title_dec = ctk.CTkLabel(self.decrypt_tab, text="فك تشفير ملف", font=ctk.CTkFont(size=18, weight="bold"))
        title_dec.grid(row=0, column=0, padx=20, pady=15)

        self.select_file_button = ctk.CTkButton(self.decrypt_tab, text="1. اختر الملف المشفر (.locked)", command=self.select_file_to_decrypt)
        self.select_file_button.grid(row=1, column=0, padx=20, pady=10, sticky="ew")
        self.locked_file_label = ctk.CTkLabel(self.decrypt_tab, text="...", text_color="gray")
        self.locked_file_label.grid(row=2, column=0, padx=20, pady=(0, 20))

        self.select_key_button = ctk.CTkButton(self.decrypt_tab, text="2. اختر ملف المفتاح (.key)", command=self.select_key_file)
        self.select_key_button.grid(row=3, column=0, padx=20, pady=10, sticky="ew")
        self.key_file_label = ctk.CTkLabel(self.decrypt_tab, text="...", text_color="gray")
        self.key_file_label.grid(row=4, column=0, padx=20, pady=(0, 20))

        self.password_entry_dec = ctk.CTkEntry(self.decrypt_tab, placeholder_text="3. أدخل كلمة المرور", show="*", height=35)
        self.password_entry_dec.grid(row=5, column=0, padx=20, pady=10, sticky="ew")
        
        self.decrypt_button = ctk.CTkButton(self.decrypt_tab, text="🔑 فك التشفير", height=45, font=ctk.CTkFont(size=18, weight="bold"), command=self.decrypt_action)
        self.decrypt_button.grid(row=6, column=0, padx=20, pady=15, sticky="ew")

    def update_status(self, status_text, processing=False):
        self.status_label.configure(text=status_text)
        if processing:
            self.progress_bar.grid(row=3, column=0, padx=20, pady=(0,5), sticky="ew")
            self.progress_bar.start()
        else:
            self.progress_bar.stop()
            self.progress_bar.grid_forget()
        self.update_idletasks()

    # --- باقي الدوال المنطقية (مع التحديثات) ---
    def select_path_to_encrypt(self):
        path = filedialog.askdirectory(title="اختر مجلدًا")
        if not path: path = filedialog.askopenfilename(title="أو اختر ملفًا واحدًا")
        if path: self.source_path = path; self.path_label_enc.configure(text=os.path.basename(path))

    def select_file_to_decrypt(self):
        path = filedialog.askopenfilename(title="اختر الملف المشفر", filetypes=[("Locked Files", "*.locked")])
        if not path: return
        self.locked_file = path; self.locked_file_label.configure(text=os.path.basename(path))
        with open(path, 'rb') as f: mode_header = f.read(1)
        if mode_header == MODE_PASSWORD_ONLY:
            self.decryption_mode = "password_only"; self.select_key_button.configure(state="disabled"); self.key_file_label.configure(text="نوع التشفير: كلمة مرور فقط")
        elif mode_header == MODE_PASSWORD_AND_KEY:
            self.decryption_mode = "password_and_key"; self.select_key_button.configure(state="normal"); self.key_file_label.configure(text="في انتظار اختيار ملف المفتاح...")
        else: messagebox.showerror("خطأ", "ملف مشفر غير معروف أو تالف."); self.decryption_mode = None

    def select_key_file(self):
        path = filedialog.askopenfilename(title="اختر ملف المفتاح", filetypes=[("Key Files", "*.key")])
        if path: self.key_file = path; self.key_file_label.configure(text=os.path.basename(path))

    def get_encryption_key(self, password, salt, key_file_content=None):
        base_secret = password.encode()
        if key_file_content: base_secret += key_file_content
        kdf = PBKDF2HMAC(algorithm=hashes.SHA256(), length=32, salt=salt, iterations=1_200_000)
        return base64.urlsafe_b64encode(kdf.derive(base_secret))

    def encrypt_action(self):
        if not self.source_path or not self.password_entry_enc.get(): messagebox.showerror("خطأ", "الرجاء اختيار ملف/مجلد وإدخال كلمة مرور."); return
        
        password = self.password_entry_enc.get(); use_keyfile = self.use_keyfile_check.get(); key_file_content = None; mode_header = MODE_PASSWORD_ONLY
        if use_keyfile:
            key_file_path = filedialog.asksaveasfilename(defaultextension=".key", filetypes=[("Key Files", "*.key")], title="احفظ ملف المفتاح في مكان آمن جدًا")
            if not key_file_path: self.update_status("الحالة: تم إلغاء التشفير."); return
            key_file_content = os.urandom(32); 
            with open(key_file_path, 'wb') as kf: kf.write(key_file_content)
            mode_header = MODE_PASSWORD_AND_KEY
        
        self.update_status("الحالة: جاري ضغط الملفات...", True)
        is_dir = os.path.isdir(self.source_path)
        if is_dir:
            temp_zip_path = shutil.make_archive("temp_archive", 'zip', self.source_path)
            with open(temp_zip_path, 'rb') as f: data_to_encrypt = f.read()
            os.remove(temp_zip_path)
        else:
            with open(self.source_path, 'rb') as f: data_to_encrypt = f.read()

        self.update_status("الحالة: جاري التشفير (قد يستغرق وقتًا)...", True)
        salt = os.urandom(16); encryption_key = self.get_encryption_key(password, salt, key_file_content); fernet = Fernet(encryption_key); encrypted_data = fernet.encrypt(data_to_encrypt)
        
        self.update_status("الحالة: جاري كتابة الملف النهائي...", True)
        output_path = self.source_path + ".locked"
        with open(output_path, 'wb') as f: f.write(mode_header); f.write(salt); f.write(encrypted_data)
        
        self.update_status("الحالة: جاهز.", False); messagebox.showinfo("نجاح!", "✅ تم التشفير بنجاح!")
        if messagebox.askyesno("تأكيد الحذف", "هل تريد حذف النسخة الأصلية غير المشفرة الآن؟\n\n🚨 تحذير: هذا الإجراء لا يمكن التراجع عنه."):
            try:
                if is_dir: shutil.rmtree(self.source_path)
                else: os.remove(self.source_path)
                messagebox.showinfo("تم الحذف", "تم حذف الأصل بنجاح.")
            except Exception as e: messagebox.showerror("خطأ حذف", f"لم نتمكن من حذف الأصل: {e}")

    def decrypt_action(self):
        if not self.locked_file or not self.password_entry_dec.get(): messagebox.showerror("خطأ", "اختر ملف وأدخل كلمة مرور."); return
        key_file_content = None
        if self.decryption_mode == "password_and_key":
            if not self.key_file: messagebox.showerror("خطأ", "هذا الملف يتطلب مفتاح."); return
            with open(self.key_file, 'rb') as kf: key_file_content = kf.read()
        
        self.update_status("الحالة: جاري فك التشفير (قد يستغرق وقتًا)...", True)
        try:
            password = self.password_entry_dec.get()
            with open(self.locked_file, 'rb') as f: f.read(1); salt = f.read(16); encrypted_data = f.read()
            encryption_key = self.get_encryption_key(password, salt, key_file_content); fernet = Fernet(encryption_key); decrypted_data = fernet.decrypt(encrypted_data)

            # --- هنا التغيير: السؤال عن مكان الحفظ ---
            self.update_status("الحالة: جاهز.", False)
            output_folder = filedialog.askdirectory(title="اختر مجلدًا لحفظ الملفات المفكوكة فيه")
            if not output_folder: self.update_status("الحالة: تم إلغاء الحفظ."); return

            output_path = os.path.join(output_folder, os.path.basename(self.locked_file).replace(".locked", ""))
            
            try: # محاولة فك الضغط
                with open("dec_temp.zip", 'wb') as f: f.write(decrypted_data)
                shutil.unpack_archive("dec_temp.zip", output_path)
                os.remove("dec_temp.zip")
            except: # إذا فشل، فهذا يعني أنه ملف واحد
                with open(output_path, 'wb') as f: f.write(decrypted_data)
            
            messagebox.showinfo("نجاح!", f"✅ تم فك التشفير بنجاح!\n\nتم الحفظ في: {output_path}")
            if messagebox.askyesno("تأكيد الحذف", "هل تريد حذف الملف المشفر (.locked) الآن؟"):
                try: os.remove(self.locked_file); messagebox.showinfo("تم الحذف", "تم حذف الملف المشفر.")
                except Exception as e: messagebox.showerror("خطأ حذف", f"لم نتمكن من حذف الملف المشفر: {e}")
        except Exception as e:
            self.update_status("الحالة: خطأ.", False); messagebox.showerror("فشل فك التشفير", "فشلت العملية. الأسباب المحتملة:\n\n- كلمة المرور غير صحيحة.\n- ملف المفتاح غير صحيح.\n- الملف تالف أو تم التلاعب به.")

if __name__ == "__main__":
    app = AegisCrypt()
    app.mainloop()
